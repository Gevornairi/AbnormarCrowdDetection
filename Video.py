...
def draw_flow_with_force(img, flow, force, step=8):
    h, w = img.shape[:2]
    y, x = np.mgrid[step / 2:h:step, step / 2:w:step].reshape(2, -1)
    fx, fy = flow[y, x].T
    lines = np.vstack([x, y, x + fx, y + fy]).T.reshape(-1, 2, 2)
    ...
    cv2.polylines(vis, lines, 0, (0, 255, 255))
    cv2.polylines(vis, lines1, 0, (0, 0, 255))
...
def get_flow(prev,next):
    ...
    next=cv2.resize(next, (0, 0), fx=2, fy=2)
    nextgray = cv2.cvtColor(next, cv2.COLOR_BGR2GRAY)
    flow = cv2.calcOpticalFlowFarneback(prevgray, nextgray, 0.5, 5, 15, 3, 5, 1.1, cv2.OPTFLOW_FARNEBACK_GAUSSIAN)
    return flow
